...
call conda install -y clangdev flang libflang llvm-meta openmp vc
...
call cd %CONDA_PREFIX%
if exist "%CONDA_PREFIX%\scipy" (call echo Warning: The folder scipy already exists, skipping the cloning process.) else (call git clone https://github.com/scipy/scipy.git)
echo NOTE: The scipy repository was cloned to %CONDA_PREFIX%\scipy
call cd scipy
...
copy c:\scipy\%WHEEL_NAME% %CONDA_PREFIX%\
:: Clean up.
call del tmp
call set WHEEL_NAME=
@echo on
